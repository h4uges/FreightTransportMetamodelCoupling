...
public class HandelModelService {
	...
	public static DataExchangeRoot loadMATSimModel(String sourceModelPath) {
		...
		return (DataExchangeRoot) resource.getContents().stream()
				.filter(elem -> elem instanceof DataExchangeRoot).findFirst().get();
	}
	
	public static SolutionExchangeRoot loadLogiToppModel(String sourceModelPath) {
		EPackage.Registry.INSTANCE.put(logiToppMetamodel.dataExchange.DataExchangePackage.eNS_URI, logiToppMetamodel.dataExchange.DataExchangePackage.eINSTANCE);
		...
		return (SolutionExchangeRoot) resource.getContents().stream().filter(elem -> elem instanceof SolutionExchangeRoot).findFirst().get();
	}
